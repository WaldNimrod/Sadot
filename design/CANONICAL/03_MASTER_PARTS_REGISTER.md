...
| Existing tree "site-plan #3" — olive (×1) | team_00 direct observation 2026-07-14: olive, 2.00m height. **Uses the site-plan sheet's own tree numbering, NOT the 2023 survey schedule** — confirmed distinct from (and not to be confused with) `existing_trees.table`'s own #3 entry (generic, 3.00m — different tree). Canopy/trunk diameter not given; Blender object uses assumed placeholder proportions, flagged as such. | SCRATCH-PROCEDURAL (trunk cylinder + canopy icosphere) — built 2026-07-14 | Low | EXIST | `blender/data/site/SITE_GEO.yaml` → `siteplan_trees` (new section, separate from `existing_trees.table` — see that section's own warning note on the two numbering schemes). Position extracted from `שטח ובית.pdf` via 3-point affine fit (0.000m residual). Objects: `TREE_siteplan03_existing_olive_trunk`/`_canopy`. |
