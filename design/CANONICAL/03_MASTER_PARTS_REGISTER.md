...
| Part | Description | Build Method | Complexity | Priority | Notes / Source |
|---|---|---|---|---|---|
| Generic existing tree (×12) | Unidentified species ("עץ" in survey), heights 2.5–6.5m, diam 0.11–0.34m — real per-tree data available. | SOURCED-CANDIDATE (generic proxy driven by real height/diameter) or SCRATCH-PROCEDURAL | Low | EXIST | `blender/data/site/SITE_GEO.yaml` — species ID needs an arborist visit; preserve-vs-remove per tree deferred to S002. |
