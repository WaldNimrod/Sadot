...
| Roof (flat, placeholder) | Dark warm gray (PLACEHOLDER — not yet client-specified) | `#59544D` | **Rebuilt 2026-07-14 (pass 11 — replaces the earlier hull-clustering `ROOF_section_<id>` pieces, rejected as imprecise).** Now 4 pieces named by real IFC storey, not arbitrary cluster id: `ROOF_ground_floor`, `ROOF_upper_floor`, `ROOF_roof6_0`, `ROOF_roof6_1`. Each wall matched to its true `IfcBuildingStorey` (not guessed clustering); each piece's footprint is a real filled union of that storey's own wall footprints (grid-rasterized + flood-filled so enclosed room interiors are solid, not a convex hull and not a hollow wall-trace); height = that storey's own max wall-top + 5cm clearance. Ground floor has the upper-floor/roof-6 footprints subtracted so it doesn't cap the double-height sections. Open deck still excluded. Color not yet client-confirmed. Full method: `blender/CURRENT_MODEL.md` pass 11. |
