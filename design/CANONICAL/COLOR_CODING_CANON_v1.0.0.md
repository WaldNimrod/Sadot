...
| Ground fill (solid) | Solid opaque brown — deliberately distinct (opaque, not transparent) from the survey-reference terrain above, since this represents real fill/build-up soil, not the abstract survey polygon | `#321E0F` (darkened 2026-07-14 from `#4D2E17` — team_00: "יותר כהה קצת בבקשה" — a bit darker please), 100% opaque | New 2026-07-14 (team_00 instruction): "יש 'למלא' אדמה ולהשלים את שכבת הterrain עד לחומה שיצרנו ממזרח... זה הקרקע שלהם הבסיסית" (need to fill soil and complete the terrain layer to the east wall — this is their base ground). First use: `GROUND_FILL_east_tree03_area`, a flat plate at 54.91m — **rejected** (not a real sculptable volume, didn't continue the terrain's actual slope). Rebuilt as `GROUND_FILL_east_solid`, a real solid but only under the east strip — **also flagged as incomplete** ("הקרקע צריכה להיות מתחת לכל המגרש," the ground needs to be under the whole plot). **Current: `GROUND_BASE_solid_full_plot`** — one closed-manifold solid (24v/33f, 0 non-manifold edges) under the ENTIRE real plot (pentagon interior + east-wall bump-out), top surface flush with the surveyor's terrain everywhere, real elevation 52.50–56.76m, ~4-4.3m of genuine sculptable depth throughout — meant to be sculpted (volume added/removed) as the first step of grading design. Same material reused throughout, `MAT_ground_fill_solid_brown` — the color itself never changed, only the geometry. See `blender/CURRENT_MODEL.md` pass 20. |
