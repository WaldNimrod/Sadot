...
| Roof (placeholder — flat concrete pieces + real-slope gables) | Dark warm gray (PLACEHOLDER — not yet client-specified) | `#59544D` | **Rebuilt 2026-07-14 twice (pass 11, then pass 12 after team_00 rejected pass 11's flat-only version).** 5 pieces, named by real IFC storey/space, not arbitrary cluster id: `ROOF_ground_floor` + `ROOF_upper_floor` (real 15° gables — slope magnitude fit from the IFC's own `רעפים` tile-roof elements, ridge along each footprint's PCA long axis), `ROOF_roof6_0`/`ROOF_roof6_1` (8° gables, matching the IFC's `Generic -15new 2` element), `ROOF_deck_porch` (flat, new in pass 12 — the round front deck was previously left unroofed, which team_00 flagged as wrong). Each footprint is a real filled union of its own walls (grid-rasterized + flood-filled — not a convex hull, not a hollow wall-trace). Single unified priority pass (`roof6 > upper > {ground, deck}`) so no XY point is double-covered or missed — verified as zero overlapping cells across all 5. Deck roof height (2.4m above the deck surface) is an unconfirmed placeholder. Color not yet client-confirmed. Full method: `blender/CURRENT_MODEL.md` pass 12. |
