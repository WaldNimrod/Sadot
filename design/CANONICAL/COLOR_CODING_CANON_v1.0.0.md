...
| Ground fill (solid) | Solid opaque brown — deliberately distinct (opaque, not transparent) from the survey-reference terrain above, since this represents real fill/build-up soil, not the abstract survey polygon | `#4D2E17`, 100% opaque | New 2026-07-14 (team_00 instruction): "יש 'למלא' אדמה ולהשלים את שכבת הterrain עד לחומה שיצרנו ממזרח... זה הקרקע שלהם הבסיסית" (need to fill soil and complete the terrain layer to the east wall — this is their base ground). First use: `GROUND_FILL_east_tree03_area`, a flat plate at 54.91m — **rejected same day** ("לא טוב," not a real sculptable volume, didn't continue the terrain's actual slope). **Rebuilt as `GROUND_FILL_east_solid`**: a real closed-manifold solid (16v/14f) whose top surface follows the terrain's own real varying elevation out to the east wall, ~4.04m of genuine depth (real Z 52.50–56.54m) meant to be sculpted (volume added/removed) as the first step of grading design. Same material reused, `MAT_ground_fill_solid_brown` — the color itself didn't change, only the geometry. See `blender/CURRENT_MODEL.md` pass 19. |
