...
**Low maintenance is the client's stated #1 priority** (repeated across multiple voice notes — see
`CLIENT_BRIEF_NIV_SADOT_v1.0.0.md` §1). This caps lawn area, plant density, and irrigation complexity — any S002/S003
palette must be screened against this before anything else. Also: avoid dense ground-cover/rock-pile arrangements
that create snake/wildlife hiding spots near paths and seating (§7 of the brief).

## Site zones & planting palette
**TBD for final zoning** — no formal zones or plant palette recorded yet. Awaits S002 concept design + the
plant-selection engine (harvested SMA crop/climate KB + the authored permaculture KB, both under `knowledge/`)
reconciled against Niv Sadot's site survey (`blender/data/site/SITE_GEO.yaml`) and the brief above.

## Planting plan (bed-by-bed / zone-by-zone)
**TBD** — no planting plan exists yet. Awaits S003 (detailed design) — the landscape-architecture equivalent of a locked crop mix.

## Irrigation demand / water budget
**TBD** — depends on the planting plan above + the irrigation spec (04). No figures recorded yet.
