...
version: 1.3.0
status: DRAFT — synthesized from client voice notes; most identity/location questions resolved by team_00 2026-07-09 (Yinon/Shani, Tasi/Pierre, deck, pool, bananas, parents' balcony); a site-understanding sketch sent to Niv for final visual confirmation; remaining §9 items still open
date: 2026-07-08 (updated 2026-07-09 twice)
...
- **Correction (2026-07-09):** the planted buffer is **between the two children's OWN pergolas (Yinon's and
  Shani's) — separating the kids from each other**, not between Niv's pergola and a child's. Vegetation, not a
  solid structure — PARTIAL privacy: block enough for privacy on one side, but do NOT fully block light/view on
  the other side. Client specifically notes the pergola in question has only one window, and enough light should
  still reach it.
- IFC re-investigation (`HOUSE_IFC_REFERENCE.md` §2) found a plausible candidate pair of entrance-floor windows
  ...
  both at the back — and team_00 confirmed 2026-07-09 this identification is accurate.
- **New (2026-07-09):** there is also a separate **parents' balcony on the second floor**, which needs some
  balcony planting/gardening (`גינון מרפסת`) — a distinct, smaller-scope planting item from the ground-floor garden.
...
- **A planned, dug swimming pool** (`בריכת שחייה חפורה`) — confirmed 2026-07-09, visible in the client's own
  hand sketch (the large curved shape). **Open question:** whether to ALSO add a separate ornamental/decorative
  pond (`ברכת נוי`, possibly with fish/plants — this is likely what the ASR rendered as "fish pond" / "ברכה
  צמחייה") in addition to the swimming pool. Not yet decided.
- **Banana circle** (a permaculture greywater/compost technique — ties directly into `knowledge/permaculture/`)
- Possibly a water feature that rises/cascades to an upper level (ASR uncertain — still not confirmed either way)
- Storage shed/structure for garden tools
- Herb + tea plants

## 5. Preferred plants (client's own list, to send reference photos separately)

Fruit trees: **avocado, mango, pecan, and carob if feasible** (clearly legible in the transcript), plus **bananas**
— confirmed by team_00 (2026-07-09) as the item the ASR garbled as "נוראים גליים" (previously incorrectly
suspected to be "guava"). General preference for edible/useful species over purely ornamental ones — consistent
with the permaculture-guild approach in `knowledge/permaculture/02_GUILDS_AND_PLANTING_STRATEGY.md`.
...
- **טאסי (Tasi)** — neighbor to the **west**, immediately adjacent house. **Separation needs to be TALL,
  specifically addressing the second floor** (the parents' balcony faces this direction — see §2a) — a low
  screen/climbing plant at ground level alone would not be sufficient.
...
- [ ] Reference photos for preferred plants (client said he would send separately — check `raw-materials/from-client/`
      for updates)
- [ ] The site-understanding sketch (`design/SITE_UNDERSTANDING_SKETCH_v1.0.0.svg`) sent to Niv for review —
      awaiting his confirmation/corrections on all placements
- [x] **RESOLVED by team_00 (2026-07-09):** "עינון"/"שני" = **Yinon and Shani, the children** — each has their own
      room + pergola + one window, at the back of the house, confirmed accurate by team_00. IFC re-investigation
      found a plausible candidate pair (entrance-floor windows, tags 5834063 + 5795233) — still not visually
      confirmed against the architect's plan, but the room identification itself is confirmed.
- [x] **RESOLVED by team_00 (2026-07-09):** Tasi (west, adjacent house, tall/2nd-floor separation) and Pierre
      (back) — see §8.
...
- [x] **RESOLVED by team_00 (2026-07-09):** the buffer/privacy planting in §2a is between the two children's own
      pergolas (separating them from each other), not between Niv and a child.
- [x] **RESOLVED by team_00 (2026-07-09):** the 5th fruit tree in §5 (ASR: "נוראים גליים") = **bananas**.
- [x] **NEW, RESOLVED by team_00 (2026-07-09):** a parents' balcony exists on the second floor, needing balcony
      planting — added to §2a.
- [x] **NEW, RESOLVED by team_00 (2026-07-09):** the sketch/pool feature is a confirmed **planned dug swimming
      pool**, not just a vague water feature — added to §4.
